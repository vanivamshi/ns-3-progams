...
/////////////////////////// Compounding MAC
  ...
  std::string q5 = "abcdefghabcdefg"; //message5
  std::string q6 = "abcdefghabcdefg"; //message6
  std::string q7 = "abcdefghabcdefg"; //message7
  std::string q8 = "abcdefghabcdefg"; //message8

  // combined message and hash
  std::string Q = q1 + q2 + q3 + q4;
  uint32_t  Q_hash = Hash32 (Q);
  //std::cout<<Q_hash<<std::endl;


  // split hash
  uint32_t H4 = Q_hash%100;
  uint32_t H3 = (Q_hash/100)%100;
  uint32_t H2 = (Q_hash/10000)%1000;
  uint32_t H1 = (Q_hash/10000000)%1000;
...
*/
  ...
  echoClient1.SetFill (clientApps1.Get (0), q1);
  ...
  echoClient2.SetFill (clientApps2.Get (0), q2);
  ...
  echoClient3.SetFill (clientApps3.Get (0), q3);
  ...
  echoClient4.SetFill (clientApps4.Get (0), q4);
  ...
  //// client5
  UdpEchoClientHelper echoClient5 (interfaces.GetAddress (1), 9);
  echoClient5.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient5.SetAttribute ("Interval", TimeValue (Seconds (1.0))); //interPacketInterval
  echoClient5.SetAttribute ("PacketSize", UintegerValue (10));
  //echoClient.SetAttribute ("txPower", UintegerValue (35));
  
  ApplicationContainer clientApps5 = echoClient1.Install (nodes.Get (0));
  echoClient5.SetFill (clientApps5.Get (0), M5);
  ...
  ApplicationContainer serverApps5 = echoServer5.Install (nodes.Get (1));
  //serverApps.SetAttribute ("txPower", UintegerValue (23));
  ...
  servermessages.push(ii.str());
  ////
  
  
  //// client6
  UdpEchoClientHelper echoClient6 (interfaces.GetAddress (1), 9);
  echoClient6.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient6.SetAttribute ("Interval", TimeValue (Seconds (1.0))); //interPacketInterval
  echoClient6.SetAttribute ("PacketSize", UintegerValue (10));
  //echoClient.SetAttribute ("txPower", UintegerValue (35));
  
  ApplicationContainer clientApps6 = echoClient6.Install (nodes.Get (0));
  echoClient6.SetFill (clientApps6.Get (0), M6);
  ...
  ApplicationContainer serverApps6 = echoServer6.Install (nodes.Get (1));
  //serverApps.SetAttribute ("txPower", UintegerValue (23));
  ...
  servermessages.push(kk.str());
  ////
  
  
  //// client7
  UdpEchoClientHelper echoClient7 (interfaces.GetAddress (1), 9);
  echoClient7.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient7.SetAttribute ("Interval", TimeValue (Seconds (1.0))); //interPacketInterval
  echoClient7.SetAttribute ("PacketSize", UintegerValue (10));
  //echoClient.SetAttribute ("txPower", UintegerValue (35));
  
  ApplicationContainer clientApps7 = echoClient7.Install (nodes.Get (0));
  echoClient7.SetFill (clientApps7.Get (0), M7);
  ...
  // server7
  UdpEchoServerHelper echoServer7 (9);

  ApplicationContainer serverApps7 = echoServer7.Install (nodes.Get (1));
  //serverApps.SetAttribute ("txPower", UintegerValue (23));
  ...
  servermessages.push(mm.str());
  ////
  
  
  //// client8
  UdpEchoClientHelper echoClient8 (interfaces.GetAddress (1), 9);
  echoClient8.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient8.SetAttribute ("Interval", TimeValue (Seconds (1.0))); //interPacketInterval
  echoClient8.SetAttribute ("PacketSize", UintegerValue (10));
  //echoClient.SetAttribute ("txPower", UintegerValue (35));
  
  ApplicationContainer clientApps8 = echoClient8.Install (nodes.Get (0));
  echoClient8.SetFill (clientApps8.Get (0), M8);
  clientApps8.Start (Seconds (2.0));
  clientApps8.Stop (Seconds (5.0));
  //std::cout<<nodes.Get(0)<<std::endl;


  // server8
  UdpEchoServerHelper echoServer8 (9);

  ApplicationContainer serverApps8 = echoServer8.Install (nodes.Get (1));
  //serverApps.SetAttribute ("txPower", UintegerValue (23));
  serverApps8.Start (Seconds (1.0));
  serverApps8.Stop (Seconds (5.0));
  //std::cout<<nodes.Get(1)<<std::endl;
  ...
  std::string OO = servermessages.top();
  std::string MM = servermessages.top();
  std::string KK = servermessages.top();
  std::string II = servermessages.top();
  ...
  II = II.substr(0, AA.size()-6);
  KK = KK.substr(0, CC.size()-6);
  MM = MM.substr(0, EE.size()-6);
  OO = OO.substr(0, GG.size()-6);
  
  std::string BB = bb.str();
  std::string DD = dd.str();
  std::string FF = ff.str();
  std::string HH = hh.str();
  std::string JJ = jj.str();
  std::string LL = ll.str();
  std::string NN = nn.str();
  std::string PP = pp.str();

  BB = BB.substr(0, BB.size()-6);
  DD = DD.substr(0, DD.size()-6);
  FF = FF.substr(0, FF.size()-6);
  HH = HH.substr(0, HH.size()-6);
  JJ = JJ.substr(0, JJ.size()-6);
  LL = LL.substr(0, LL.size()-6);
  NN = NN.substr(0, NN.size()-6);
  PP = PP.substr(0, PP.size()-6);
